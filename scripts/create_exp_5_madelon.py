...
OPTIMIZE_ROOT = PROJECT_ROOT / "input" / "optimize" / "exp" / "exp_5_madelon_reg1e-1_mfull"
...
def _generate_config() -> dict:
    return {
        "task": "generate_data",
        "run_name": "madelon_reg1e-1_mfull",
        "seed": 0,
        "problem": {
            "type": "logistic",
        ...
        "save": {
            "path": "data/generated/exp/madelon_reg1e-1_mfull.npz",
        },
    }
...
def _problem_block() -> dict:
    return {
        "problem": {
            "type": "logistic",
            "source": "data/generated/exp/madelon_reg1e-1_mfull.npz",
        },
        "initialization": {
            "type": "zeros",
        },
...
def _rs_cn_config(s: int, seed: int) -> dict:
    run_name = f"rs_cn_madelon_reg1e-1_mfull_s{s}_seed{seed}"
    optimizer = {
        "type": "rs_cn",
...
def _ars_cn_config(s: int, r_value: int, t_value: int, seed: int) -> dict:
    run_name = f"ars_cn_madelon_reg1e-1_mfull_s{s}_r{r_value}_t{t_value}_seed{seed}"
    optimizer = {
        "type": "ars_cn",
...
def _rs_rn_config(s: int, seed: int) -> dict:
    run_name = f"rs_rn_madelon_reg1e-1_mfull_s{s}_seed{seed}"
    optimizer = {
        "type": "rs_rn",
        "max_iter": RS_MAX_ITER,
        "tol": float(TOL),
...
def _plot_config(y_key: str, yscale: str, save_name: str) -> dict:
        ...
        for r_value in ARS_R_VALUES:
            for t_value in ARS_T_VALUES:
                ...
                ars_inputs.append(
                    {
                        "paths": _group_paths(
                            f"ars_cn_madelon_reg1e-1_mfull_s{s}_r{r_value}_t{t_value}"
                        ),
    ...
    return {
        "task": "plot",
        "plot_name": f"exp_5_madelon_reg1e-1_mfull_{ylabel}",
        "inputs": [
            {
                "paths": _group_paths("rs_cn_madelon_reg1e-1_mfull_s100"),
                "aggregate": {"center": "mean", "band": "minmax", "alpha": 0.16},
                "label": "RS-CN s=100",
                "color": "#495057",
                "linestyle": "dashdot",
            ...
            {
                "paths": _group_paths("rs_cn_madelon_reg1e-1_mfull_s200"),
                "aggregate": {"center": "mean", "band": "minmax", "alpha": 0.16},
                "label": "RS-CN s=200",
                "color": "#212529",
                "linestyle": "solid",
            ...
            *ars_inputs,
            {
                "paths": _group_paths("rs_rn_madelon_reg1e-1_mfull_s100"),
                "aggregate": {"center": "mean", "band": "minmax", "alpha": 0.16},
                "label": "RS-RN s=100",
                "color": "#fd7e14",
                "linestyle": "dotted",
            ...
            {
                "paths": _group_paths("rs_rn_madelon_reg1e-1_mfull_s200"),
                "aggregate": {"center": "mean", "band": "minmax", "alpha": 0.16},
                "label": "RS-RN s=200",
                "color": "#e8590c",
                "linestyle": "dotted",
        ...
        "plot": {
            ...
            "xscale": "linear",
            "yscale": yscale,
            "title": "exp_5: madelon reg=1e-1 m=full",
        ...
        "steps": [
            {
                ...
                "config": "input/generate_data/exp/madelon_reg1e-1_mfull.yml",
            },
            {
                "command": "optimize_parallel",
                "max_parallel": AUTO_MAX_PARALLEL,
                "configs": [
                    f"input/optimize/exp/exp_5_madelon_reg1e-1_mfull/rs_cn_s100_seed{seed}.yml"
                    for seed in SEEDS
                ],
            },
            {
                ...
                "max_parallel": AUTO_MAX_PARALLEL,
                "configs": [
                    f"input/optimize/exp/exp_5_madelon_reg1e-1_mfull/rs_cn_s200_seed{seed}.yml"
                    for seed in SEEDS
                ],
            },
            *[
                {
                    ...
                    "configs": [
                        (
                            "input/optimize/exp/exp_5_madelon_reg1e-1_mfull/"
                            f"ars_cn_s{s}_r{r_value}_t{t_value}_seed{seed}.yml"
                        )
                        for seed in SEEDS
                    ],
                }
            ...
            ],
            {
                ...
                "max_parallel": AUTO_MAX_PARALLEL,
                "configs": [
                    f"input/optimize/exp/exp_5_madelon_reg1e-1_mfull/rs_rn_s100_seed{seed}.yml"
                    for seed in SEEDS
                ],
            },
            {
                ...
                "max_parallel": AUTO_MAX_PARALLEL,
                "configs": [
                    f"input/optimize/exp/exp_5_madelon_reg1e-1_mfull/rs_rn_s200_seed{seed}.yml"
                    for seed in SEEDS
                ],
            },
            {
                "command": "plot",
                "config": "input/plot/exp/exp_5_madelon_reg1e-1_mfull_grad_norm.yml",
            },
            {
                "command": "plot",
                "config": "input/plot/exp/exp_5_madelon_reg1e-1_mfull_function_value.yml",
            },
        ],
    }
...
def main() -> None:
    ...
    save_yaml(_generate_config(), GENERATE_ROOT / "madelon_reg1e-1_mfull.yml")
    ...
    save_yaml(
        _plot_config("grad_norm", "log", "exp_5_madelon_reg1e-1_mfull_grad_norm.png"),
        PLOT_ROOT / "exp_5_madelon_reg1e-1_mfull_grad_norm.yml",
    )
    save_yaml(
        _plot_config("f", "linear", "exp_5_madelon_reg1e-1_mfull_function_value.png"),
        PLOT_ROOT / "exp_5_madelon_reg1e-1_mfull_function_value.yml",
    )

    pipeline = _pipeline_config()
    save_yaml(pipeline, PIPELINE_ROOT / "exp_5.yml")
    save_yaml(pipeline, PIPELINE_ROOT / "exp_5_madelon_reg1e-1_mfull.yml")
